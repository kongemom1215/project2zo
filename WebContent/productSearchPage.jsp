...
<link rel="stylesheet" type="text/css" href="css/productadmin.css?ver=104">
				...
				<div id="select">
					<form name="frm" action="productSearch.do" onsubmit="return chk()">
						<select name="option" class="option">
							...
							<option value="pid" <c:if test="${option eq 'pid'}">selected="selected"</c:if>>상품번호</option>
							<option value="pname" <c:if test="${option eq 'pname'}">selected="selected"</c:if>>상품이름</option>
						</select>
				...
				<div id="search_table">
					<table>
						<c:forEach var="product" items="${productlist }">
							<tr>
								...
								<td rowspan="2"><a href="productInfo.do?pid=${product.pid }&pageNum=${pageNum}&option=${option}&search_value=${search_value}"><img src="${product.pthumbimg }" id="img2"></a></td>
					...
					<div style="text-align : center">
						<c:if test="${startPage>blockSize }">
							<a href="productSearch.do?pageNum=${startPage-blockSize }&option=${option}&search_value=${search_value}">[이전]</a>
						</c:if>
						<c:forEach var="i" begin="${startPage }" end="${endPage}">
							<c:choose>
								<c:when test="${i eq pageNum }">
									<a href="productSearch.do?pageNum=${i }&option=${option}&search_value=${search_value}" style="font-weight:bold;">${i }</a>
								</c:when>
								<c:otherwise>
								<a href="productSearch.do?pageNum=${i }&option=${option}&search_value=${search_value}">${i }</a>
								</c:otherwise>
							</c:choose>	
						</c:forEach>
						<c:if test="${endPage< pageCnt}">
							<a href="productSearch.do?pageNum=${startPage+blockSize }&option=${option}&search_value=${search_value}">[다음]</a>
						</c:if>
					</div>
					<br>
					<div>
