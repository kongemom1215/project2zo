...
<link rel="stylesheet" type="text/css" href="css/notice.css?ver=113"/>
			...
			<div id="content2">
				<div id="content3">
					...
					<table style="text-align:center;">
						...
						<tr>
							<td class="search" style="text-align=center;" colspan="3">
								<form action="reviewSearch.do" name="frm" onsubmit="return chk()">
								<input type="text" name="pid" value="${pid }" required="required">
					...
					<table>
						...
						<c:forEach var="review" items="${reviewlist }">
							<tr>
								<td><c:if test="${not empty review.rcmt }">
									[답변완료]</c:if>
									<a href="reviewInfo.do?rid=${review.rid }&pageNum=${pageNum}&rctg=search">${review.rtitle }</a></td>
								<td>${review.rwriter }</td>
								<td><fmt:formatDate value="${review.rdate }" pattern="yyyy-MM-dd"/></td>
								<td>${review.rhit }</td>
							</tr>
						</c:forEach>
					</table>
					<div style="text-align:center">
						<c:if test="${startPage>blockSize }">
							<a href="reviewSearch.do?pageNum=${startPage-blockSize }">[이전]</a>
						</c:if>
						<c:forEach var="i" begin="${startPage }" end="${endPage}">
							<a href="reviewSearch.do?pageNum=${i }">[${i }]</a>
						</c:forEach>
						<c:if test="${endPage< pageCnt}">
							<a href="reviewSearch.do?pageNum=${startPage+blockPage }">[다음]</a>
						</c:if>
					</div>
				</div>
			</div>
