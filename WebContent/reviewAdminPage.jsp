...
<link rel="stylesheet" type="text/css" href="css/notice.css?ver=111"/>
				...
				<div id="content3">
						...
						<c:forEach var="review" items="${reviewlist }">
							<tr>
								<td><c:if test="${not empty review.rcmt }">
									[답변완료]</c:if>
									<a href="reviewInfo.do?rid=${review.rid }&pageNum=${pageNum}">${review.rtitle }</a></td>
					...
					<div style="text-align:center">
						<c:if test="${startPage>blockSize }">
							<a href="reviewAdminPage.do?pageNum=${startPage-blockSize }">[이전]</a>
						</c:if>
						<c:forEach var="i" begin="${startPage }" end="${endPage}">
							<a href="reviewAdminPage.do?pageNum=${i }">[${i }]</a>
						</c:forEach>
						<c:if test="${endPage< pageCnt}">
							<a href="reviewAdminPage.do?pageNum=${startPage+blockPage }">[다음]</a>
						</c:if>
					</div>
				</div>
			</div>
